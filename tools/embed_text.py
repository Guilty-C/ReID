...
def main():
    ...
    # Prepare grouped texts per image
    grouped_texts = []
    image_names = []
    ...
    flat_texts = [t for group in grouped_texts for t in group]
    total_batches = (len(flat_texts) - 1) // batch_size + 1 if len(flat_texts) > 0 else 0
    ...
    for i in range(0, len(flat_texts), batch_size):
        ...
        flat_embeds_chunks.append(batch_embeds)
    ...
    # Concatenate all embeddings for reconstruction
    if flat_embeds_chunks:
        flat_embeds = np.concatenate(flat_embeds_chunks, axis=0).astype("float32")
    else:
        flat_embeds = np.zeros((0, 768), dtype="float32")
    
    # Reconstruct per-image embeddings by averaging multiple captions
    all_embeds = np.zeros((len(grouped_texts), flat_embeds.shape[1] if flat_embeds.size else 768), dtype="float32")
    ...
    for i, group in enumerate(grouped_texts):
        count = len(group)
        if count == 0:
            all_embeds[i] = np.zeros((flat_embeds.shape[1] if flat_embeds.size else 768,), dtype="float32")
        elif count == 1:
            all_embeds[i] = flat_embeds[idx]
            idx += 1
        else:
            grp = flat_embeds[idx: idx+count]
            # compute weights per token and normalize
            raw_w = np.array([_token_weight(tok) for tok in group], dtype="float32")
            s = float(raw_w.sum())
            w = (raw_w / s) if s > 1e-9 else np.ones_like(raw_w) / max(count, 1)
            weighted = (grp * w[:, None]).sum(axis=0)
            # renormalize
            norm = np.linalg.norm(weighted) + 1e-9
            all_embeds[i] = (weighted / norm).astype("float32")
            idx += count
    ...
    assert all_embeds.shape[0] == len(grouped_texts), f"Shape mismatch: {all_embeds.shape[0]} vs {len(grouped_texts)}"
